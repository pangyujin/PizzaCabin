    ...
    <div class="main">
      <div class="sidebar">
        <ul>
          ...
          <li><a class="active" href="sides.php"><img src="../static/img/icon/fries_icon.png" alt="sides" style="height: 50px; width: 50px;"><br />Sides</a></li>
      ...
      <div class="container">
        ...
        <?php
          $sides_query = "SELECT * 
                          ...
                          WHERE F.category = 'sides' ";
          $sides_result = $db->query($sides_query);

          $sides_list = array();
          for($i=0; $i<$sides_result->num_rows; $i++) {
            $row = $sides_result->fetch_assoc();
            $item = new FoodDAO($row['category'], $row['name'], $row['photo'], $row['price'], $row['description']);
            $sides_list[] = $item;
          }

          foreach($sides_list as $sides) {
            $sides->render_food();
          }
        ?>
